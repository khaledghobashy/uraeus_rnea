...
def kinematic_scan(
    ...
    v_n = jnp.linalg.solve(jac, -vel_eqn_dt(x_n, t))
    a_n = jnp.linalg.solve(jac, -acc_eqn_dt(x_n, t))

    return (x_n, v_n, a_n, dt), (x_p, v_p, a_p)


@partial(jax.jit, static_argnums=(0,))
def kinematic_sim_scan(
    model: MultiBodyGraphData,
    x0: np.ndarray,
    t_array: np.ndarray,
):
...
# @partial(jax.jit, static_argnums=(0,))
def kinematic_sim_loop(model: MultiBodyGraphData, x0: np.ndarray, t_array: np.ndarray):
    qdt0s = [x0]
    qdt1s = [0 * x0]
    qdt2s = [0 * x0]
    ...
    for i, t in enumerate(t_array):
        ...
        qdt1 = jnp.linalg.solve(jac, -vel_eqn_dt(qdt0, t))
        qdt2 = jnp.linalg.solve(jac, -acc_eqn_dt(qdt0, t))
