...
from uraeus.rnea.quaternion.models.analytic_double_pendulum import analytic_system
...
class DoublePendulumTest(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.multibody_system = self._build_multibody_system(self.l1, self.l2)
    ...
    def _build_multibody_system(self, l1, l2) -> Model:
        tree = MultiBodyTree("pendulum")

        l1_data = RigidBodyData(
            np.array([0, 0, -l1]), np.array([1, 0, 0, 0]), 1, np.eye(3)
        )
        l2_data = RigidBodyData(
            np.array([0, 0, -l2 - l1]), np.array([1, 0, 0, 0]), 1, np.eye(3)
        )
